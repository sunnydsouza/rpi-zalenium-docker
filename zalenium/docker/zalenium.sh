...
CONTAINER_NAME="sunnyd/zalenium"
SELENIUM_IMAGE_NAME=${SELENIUM_IMAGE_NAME:-"elgalu/selenium"}
#SELENIUM_IMAGE_NAME=${SELENIUM_IMAGE_NAME:-"deinchristian/rpi-selenium-node-chrome"}
...
ZALENIUM_ARTIFACT="$(pwd)/zalenium-3.141.59z.jar"
...
DockerTerminate()
{
    ...
    if [ "$SEND_ANONYMOUS_USAGE_INFO" = true ]; then
        ...
        local args=(
            ...
            --data an="zalenium"
            --data av="zalenium-3.141.59z.jar"
            --data cd="$ZALENIUM_STOP_COMMAND"
            --data sc="end"
            --data ds="docker"
        )

        if [[ "zalenium-3.141.59z.jar" == *"SNAPSHOT"* ]]; then
